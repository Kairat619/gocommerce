...
export default function AttributeFormActions({ isEdit, processing, errorCount, onSubmit, onSubmitAndContinue }) {
  return (
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm" role={errorCount > 0 ? "alert" : undefined}>
          {errorCount > 0 ? (
            ...
            <span className="text-gray-500">{isEdit ? "Editing attribute" : "New attribute"}</span>
          )}
        </p>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            ...
            {processing ? "Saving…" : isEdit ? "Save changes" : "Create attribute"}
          </button>
        </div>
      </div>
    </div>
