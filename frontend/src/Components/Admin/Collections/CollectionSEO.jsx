...
export default function CollectionSEO({ form, setField, errors, isEdit, originalSlug }) {
  const slug = form.url_key ? slugify(form.url_key) : slugify(form.name);
  const origin = typeof window === "undefined" ? "" : window.location.origin;

  const title = form.meta_title.trim() || form.name.trim() || "Collection name";
  ...
  return (
    <FormCard
      ...
      description="How this collection appears in search results."
    >
      <div className="space-y-4">
        <TextInput
          ...
          prefix="/collections/"
          placeholder={slugify(form.name) || "collection-name"}
          autoComplete="off"
          hint={
            isEdit
              ? "Changing this changes the collection's public address."
              : "Generated from the collection name — edit it if you like."
          }
        />

        {slugChanged && (
          <p className="rounded-md bg-amber-50 px-3 py-2 text-xs text-amber-800">
            The public address will change from <span className="font-mono">/collections/{originalSlug}</span>{" "}
            to <span className="font-mono">/collections/{slug}</span>. Existing links to the old address will
            stop working.
          </p>
        )}
        ...
        <TextInput
          ...
          hint="Falls back to the collection name. Around 60 characters reads best."
        />
        ...
        <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
          ...
          <p className="mt-0.5 line-clamp-2 text-sm text-gray-600">
            {description || "Add a meta description or a collection description to control this snippet."}
          </p>
        </div>
      </div>
    </FormCard>
